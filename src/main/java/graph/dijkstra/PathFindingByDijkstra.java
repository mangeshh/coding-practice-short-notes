...
public class PathFindingByDijkstra {
    ...
    void dijkstra(WeightedNode node) {
        ...
        while (!queue.isEmpty()) {
            WeightedNode presentNode = queue.remove(); //"remove" graph.node with minimum distance from queue
            for (WeightedNode neighbor : presentNode.getNeighbors()) { //for each neighbor
                if (!queue.contains(neighbor)) { //if neighbor is not visited
                    ...
                    if (neighbor.getDistance() > (presentNode.getDistance() + presentNode.getWeightMap().get(neighbor))) { // WeightMap.get(neighbor)) gives the edge's weight
        ...
        //print table of graph.node with minimum distance and shortest path from source
